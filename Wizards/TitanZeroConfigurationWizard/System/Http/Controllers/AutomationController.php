...
class AutomationController extends Controller
{
    ...
    public function __construct(public AutomationService $automationService, protected TitanCoreAIService $titanCoreAIService)
    {
    ...
    {
        // API key is now managed by TitanCoreAIService, no direct key needed here.
        $apikeyPart1 = base64_encode('dummy_part1');
        $apikeyPart2 = base64_encode('dummy_part2');
        $apikeyPart3 = base64_encode('dummy_part3');
        $apiUrl = base64_encode('titan-core-ai-service'); // Indicate using TitanCoreAIService
    ...
    {
        // API key is now managed by TitanCoreAIService, no direct key needed here.
        // Return dummy values for compatibility with existing UI that expects these.
        return [
            base64_encode("dummy_part1"),
            base64_encode("dummy_part2"),
            base64_encode("dummy_part3"),
            base64_encode("titan-core-ai-service"),
        ];
    }
    ...
    public function generateCampaignContent(Request $request)
    {
        ...
        try {
            ...
            $response = $this->titanCoreAIService->generate(
                ...
                [], // No tools
                'openai', // Provider
                $this->setting->openai_default_model // Model
            );
    ...
    public function generateCampaignTopics(Request $request)
    {
        ...
        try {
            ...
            $response = $this->titanCoreAIService->generate(
                "Generate only a list of max 10 topics, of $campaign_name campaign. Must resut as array json data. Result format is [topic1, topic2, ..., topicn]. Ensure that the result does not contain backticks (\`) or the string \"```json\".",
                [], // No prior messages
                [], // No tools
                'openai', // Provider
                $this->setting->openai_default_model // Model
